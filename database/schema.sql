...
CREATE INDEX media_asset_owner_idx ON media_asset (owner_id);
...
CREATE INDEX worker_profile_open_idx ON worker_profile (open_to_work) WHERE is_public;
...
CREATE TABLE social_link (
  id         uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  worker_id  uuid NOT NULL REFERENCES worker_profile(user_id) ON DELETE CASCADE,
  platform   social_platform NOT NULL,
  url        text NOT NULL,
  sort_order int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX social_link_worker_idx ON social_link (worker_id, sort_order);

CREATE TABLE experience (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  worker_id       uuid NOT NULL REFERENCES worker_profile(user_id) ON DELETE CASCADE,
  company_id      uuid REFERENCES company(id) ON DELETE SET NULL,  -- si la empresa existe en la plataforma
...
CREATE INDEX experience_worker_idx  ON experience (worker_id, sort_order);
CREATE INDEX experience_company_idx ON experience (company_id) WHERE company_id IS NOT NULL;
...
CREATE INDEX education_worker_idx ON education (worker_id, sort_order);
...
CREATE INDEX certification_worker_idx ON certification (worker_id, sort_order);
...
CREATE INDEX project_worker_idx ON project (worker_id, sort_order);
...
CREATE INDEX worker_skill_skill_idx ON worker_skill (skill_id);  -- buscar candidatos por skill
...
CREATE INDEX experience_skill_skill_idx ON experience_skill (skill_id);
...
CREATE INDEX project_skill_skill_idx ON project_skill (skill_id);
...
CREATE INDEX job_posting_company_idx   ON job_posting (company_id, status);
CREATE INDEX job_posting_published_idx ON job_posting (published_at DESC) WHERE status = 'published';
...
CREATE INDEX job_posting_skill_skill_idx ON job_posting_skill (skill_id);
...
CREATE INDEX job_application_worker_idx  ON job_application (worker_id, applied_at DESC);
CREATE INDEX job_application_posting_idx ON job_application (job_posting_id, status);
...
CREATE TABLE job_application_event (
  ...
  changed_by     uuid REFERENCES app_user(id) ON DELETE SET NULL,  -- reclutador o el propio trabajador (withdrawn)
  from_status    application_status,                               -- NULL en el evento inicial
  to_status      application_status NOT NULL,
  note           text,
  created_at     timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX job_application_event_app_idx ON job_application_event (application_id, created_at);
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'app_user', 'worker_profile', 'company', 'experience', 'education',
    'certification', 'project', 'job_posting', 'job_application'
  ] LOOP
    EXECUTE format(
